...
export class LoadBoardsService {
  boards: { [key: number]: number[] } = {
...
size: number = 0;
  ...
  constructor() { 
   this.size = Object.keys(this.boards).length;
  }

 

 loadRandomBoard(inputs: NodeListOf<HTMLInputElement>) {
    let index = Math.floor(Math.random()*this.size);
    let savedBoard = this.boards[index];
    
    for(let i=0; i<savedBoard.length; i++) {
        if(savedBoard[i] == 0) {
             inputs[i].value = ""
        } else {
            inputs[i].value = savedBoard[i].toString();
        }
    }
    savedBoard = []
}
